...
function createFood() {
    ...
    while (!foodCreated) { //пока еду не создали
        ...
        // проверка на змейку
        if (!food_cell_classes.includes('snake-unit') && !food_cell_classes.includes('barrier')) {
            var classes = '';
...
function createBarrier() {
    ...
    while (!barrierCreated) { //пока преграду не создали
        // рандом
        ...
        if (!barrier_cell_classes.includes('snake-unit') && !barrier_cell_classes.includes('food-unit')) {
            ...
            barrier_cell.setAttribute('class', classes + 'barrier');
            barrierCreated = true;
        }
    }
}
